...
def test_get_next_page_url(responses, stream):
    ...
    assert (
        actual
        == "/v1/organizations?page%5Bnumber%5D=2&page%5Bsize%5D=25&sort=updated_at"
    )
...
def test_parse_response_does_not_return_records_earlier_than_the_stop_point(
    responses, stream
):
    ...
    records = list(
        stream.parse_response(
            responses["orgs_records_before_and_after_2020-01-01.json"]
        )
    )
...
def test_get_next_page_token_returns_null_if_response_has_no_data_records(
    responses, stream
):
...
def test_get_next_page_token_returns_next_page(responses, stream):
    ...
    actual = stream.get_next_page_token(
        responses["orgs_records_before_and_after_2020-01-01.json"], None
    )
